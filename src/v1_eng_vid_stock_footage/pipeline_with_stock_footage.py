...
async def generate_script_and_keywords(api_key):
    ...
    prompt = (
        ...
        "Write a multi-sentence, 20-second engaging fact about Biology or Animals. "
        "The tone should be enthusiastic and mind-blowing. "
        "AFTER the script, add a separator '|||' followed by a visual search query for a stock video site "
        "that describes the visual background for this script ."
        "\n\nFormat: [Script] ||| [Search Term] \n"
        "(e.g. `This is the example script ... about cute cats ||| cute cats snuggling`)"
    )
